...
//test for Athena ApiGateway
type AthenaSubscriber []string

// Hosts implements the subscriber interface
func (s FixedSubscriber) Hosts() ([]string, error) { return s, nil }

func (a AthenaSubscriber) Hosts() ([]string, error) { return a, nil }

func (a AthenaSubscriber) Change(host string) ([]string, error) {
	a = append(a, host)
	return a, nil
}
...
func AthenaSubscriberFactory(cfg *config.Backend) Subscriber {
	return AthenaSubscriber(cfg.Host)
}
